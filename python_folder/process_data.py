...
# Read all judgments (full court orders)
...
for filename in judgment_files:
    ...
    # Assign labels using category folders
    for sentence in sentences:
        sentence = clean_text(sentence)  # ✅ Clean sentence before processing
        ...
        for category, folder in CATEGORY_FOLDERS.items():
            ...
            if os.path.exists(file_path):
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    category_text = clean_text(f.read())  # ✅ Clean file content before comparison
...
# Reduce "O" samples (Keep only 2000)
df_O = df[df["label"] == "O"].sample(n=2000, random_state=42)

# Oversample underrepresented categories (increase to 800 each)
df_balanced = df[df["label"] != "O"]
oversampled_dfs = [df_balanced]  # Create a list to store oversampled data

for label in ["FACTS", "JUDGMENT", "ARGUMENT", "STATUTE", "ANALYSIS"]:  # ✅ Includes ANALYSIS
