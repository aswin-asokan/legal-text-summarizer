...
def get_best_answer(question, context):
    ...
    for chunk in chunks:
        inputs = tokenizer.encode_plus(question, chunk, add_special_tokens=True, max_length=512, truncation=True, return_tensors='pt')
        ...
        start_index, end_index = torch.argmax(start_scores).item(), torch.argmax(end_scores).item() + 1

        if start_index >= end_index or end_index - start_index > 50:
        ...
        if score > highest_score and answer.strip():
            highest_score, best_answer = score, answer

    return best_answer if best_answer else "No answer found."

# Example Usage
question = "What articles are mentioned here?"
context = """
...
print(get_best_answer(question, context))
